    ...
    <div id="content">
      <div id="content-inner">
        <div class="section-card">
          ...
          <p class="section-desc">修改后将自动退出，需要重新登录。</p>
          <div class="field-group">
            <label class="field-label">新密码</label>
            <div class="pw-wrap">
              <input v-model="newPassword" :type="showNew?'text':'password'" class="field-input" placeholder="输入新密码" autocomplete="new-password"/>
          ...
          <div class="field-group">
            ...
            <div class="pw-wrap">
              <input v-model="currentPassword" :type="showCurrent?'text':'password'" class="field-input" :class="{error:errorMsg}" placeholder="输入当前密码确认" autocomplete="current-password" @keydown.enter="save"/>
          ...
          <button class="save-btn" @click="save" :disabled="saving">{{ saving?'保存中…':'保存修改' }}</button>
        </div>
...
async function save() {
  ...
  if (newPassword.value === currentPassword.value) { errorMsg.value = '新密码不能与当前密码相同'; return }
  saving.value = true
  try {
    ...
    if (!r.ok) { errorMsg.value = data.error || '修改失败'; return }
    toast('✅ 密码已更新，即将跳转登录页')
    setTimeout(async () => {
      await fetch('/api/auth/logout', { method: 'POST', credentials: 'include' })
      location.href = '/login'
    }, 1200)
  } catch { errorMsg.value = '网络错误，请重试' }
...
<style scoped>
#page { display:flex; flex-direction:column; height:100vh; height:100dvh; background:var(--bg); }
#topbar {
  display:flex; align-items:flex-end; gap:8px;
  min-height:calc(56px + var(--safe-top,0px));
  padding-bottom:8px; padding-top:max(8px,var(--safe-top,0px));
  padding-left:max(12px,var(--safe-left,0px)); padding-right:max(12px,var(--safe-right,0px));
  background:var(--bg2); border-bottom:1px solid var(--border); flex-shrink:0;
}
.topbar-title { flex:1; font-size:15px; font-weight:600; color:var(--text2); text-align:center; }
#content { flex:1; overflow-y:auto; padding:32px max(20px,calc((100vw - 520px)/2)) 60px; }
#content-inner { max-width:520px; margin:0 auto; display:flex; flex-direction:column; gap:20px; }
.section-card { background:var(--bg2); border:1px solid var(--border); border-radius:16px; padding:28px; }
.section-title { font-size:16px; font-weight:700; color:var(--text); margin-bottom:6px; }
...
.save-btn {
  ...
  font-size:15px; font-weight:600; font-family:var(--font); cursor:pointer; transition:opacity .15s;
}
.save-btn:hover:not(:disabled) { opacity:.88; }
.save-btn:disabled { opacity:.6; cursor:not-allowed; }
.danger-zone { border-color:#fecaca; }
.logout-btn {
  padding:10px 20px; border:1.5px solid var(--danger); border-radius:10px;
  background:transparent; color:var(--danger);
  font-size:14px; font-weight:600; font-family:var(--font); cursor:pointer; transition:background .15s,color .15s;
}
.logout-btn:hover { background:var(--danger); color:#fff; }
@media (max-width:768px) { #content { padding:20px 16px 60px; } .section-card { padding:20px; } }
</style>
