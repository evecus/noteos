...
func (d *DB) GetUsername() (string, error) {
	var username string
	err := d.conn.QueryRow(`SELECT username FROM credentials LIMIT 1`).Scan(&username)
	if err == sql.ErrNoRows {
...
// UpdateCredential replaces username and/or password.
// Pass empty string to keep the existing value unchanged.
func (d *DB) UpdateCredential(oldUsername, newUsername, newPassword string) error {
	if newUsername == "" && newPassword == "" {
		return nil
	}
	currentHash, err := d.GetCredentialHash(oldUsername)
	if err != nil {
		return fmt.Errorf("user not found")
	}
	targetUsername := oldUsername
	if newUsername != "" {
	...
	defer tx.Rollback()
	if _, err := tx.Exec(`DELETE FROM credentials WHERE username=?`, oldUsername); err != nil {
		return err
	}
	if _, err := tx.Exec(`INSERT INTO credentials(username, hash) VALUES(?,?)`, targetUsername, targetHash); err != nil {
		return err
	}
	return tx.Commit()
}
